...
void ssort(int * arr, int size)
{
  ...
  //First for loop iterates through the array one time

  int ind; //index for iterating through the array one time
  int indNest; //index for iterating through the array multiple times in the nested loop
  int smallNum; //The smallest number for that iteration  
  int nextNum; //The number next in the iteration being compared
  int smallNumInd; //The index for the smallest number
  ...
  for (ind = 0; ind < (size - 1); ind++)
    {
      ...
      for (indNest = ind + 1; indNest < size; indNest++)
	{
	  nextNum = arr[indNest];
